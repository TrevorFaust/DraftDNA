-- Per-range FG attempts = fg_made_* + fg_missed_* + fg_blocked_* (and hyphen key variants).
-- Aggregate fg_blocked split across distance bands by (made+miss) weight when no per-range fg_blocked_*.
-- GREATEST(makes, COALESCE(fga_*, fg_att_*, computed_att)); band att_* is NULL when no band activity so DB fga/fg_att columns are not masked by 0.
