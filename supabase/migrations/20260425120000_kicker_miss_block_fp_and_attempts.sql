-- FG misses + blocks: -1 fantasy pt each (add-on to kick_fp).
-- Attempts per short bucket: fga_* else sum(made+missed) per band; else n0039 + short misses + fg_blocked when no long-FG makes.
-- Aggregate fg_missed used only when no per-range fg_missed_* values are present.
...
COMMENT ON FUNCTION public.kicker_weekly_derived_json(jsonb) IS
  'Kicker weekly JSON: FG makes by distance, att from fga/made+miss, kick_fp includes -1 per fg_missed_* (or fg_missed) and fg_blocked.';
