-- Weight baseline rankings heavily so expert consensus dominates over user rankings.
-- Baseline = 50 votes, each user ranking = 1 vote. Ensures new baselines (e.g. PPR/season/non-sf)
-- are reflected even when existing user_rankings have a different order.
...
AS $$
  ...
  weighted_ranks AS (
    SELECT player_id, r, 50::numeric AS w FROM baseline_ranks
    UNION ALL
