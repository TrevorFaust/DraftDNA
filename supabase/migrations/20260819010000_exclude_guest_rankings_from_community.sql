-- Guest rankings are session/device-only and must not vote in community consensus.
-- Keep p_exclude_guest_session_id on the signature so existing RPC callers still work.
...
AS $$
  ...
  user_ranks AS (
    ...
    WHERE ur.league_id IS NOT NULL
      AND ur.scoring_format = p_scoring_format
      AND ur.league_type = p_league_type
      AND ur.is_superflex = p_is_superflex
      AND ur.rookies_only = false
