-- Self-heal missing human pick deadlines inside mp_tick_draft.
-- (Function body applied via MCP; this file keeps repo migrations in sync.)
...
BEGIN
  ...
  IF v_draft.pick_timer > 0 AND v_draft.pick_deadline_at IS NULL THEN
    UPDATE public.multiplayer_drafts
    SET pick_deadline_at = now() + make_interval(secs => v_draft.pick_timer)
    WHERE id = p_draft_id
