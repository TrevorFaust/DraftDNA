...
import {
  fetchDefensiveRookies2026,
  type DefensiveRookie,
} from '@/utils/defensiveRookiesPool';
...
type Props = {
  value: DefensiveRookie | null;
  onChange: (player: DefensiveRookie | null) => void;
  disabled?: boolean;
  placeholder?: string;
  className?: string;
...
export function DefensiveRookieSearchCombobox({
  value,
  onChange,
  disabled = false,
  placeholder = 'Search defensive rookies…',
  className,
  ...
  const [pool, setPool] = useState<DefensiveRookie[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const rows = await fetchDefensiveRookies2026();
      if (!cancelled) {
        setPool(rows);
        setLoading(false);
      }
        ...
        <Command shouldFilter={false}>
          ...
          {search.trim() ? (
            <CommandList className="scrollbar-thin">
              <CommandEmpty>
                {loading ? 'Loading…' : filtered.length === 0 ? 'No defensive rookies found' : ''}
              </CommandEmpty>
              <CommandGroup>
                {filtered.map((player) => (
                  <CommandItem
