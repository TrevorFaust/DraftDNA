...
**Season Predictions.** Pre Season tool where you pick a winner for every 2026 regular-season game, week by week or by team. Finish the slate and you get projected division standings (W/L, division record, PCT) with NFL-style playoff seeding, then play out a full AFC/NFC bracket through the Super Bowl. Along the way you also pick award winners: MVP, OPOY, DPOY, OROY, DROY, and Coach of the Year. OROY uses fantasy skill rookies, DROY uses 2026 defensive rookies, DPOY searches the current `rosters_2026` defense (vets and rookies), and Coach of the Year pulls from `team_coaching_2026`. Signed-in picks sync to your account across devices; guests stay in the browser only. Picks lock after September 11, 2026. Random fill only covers games you have not picked yet.
...
Roster and weekly stats: `npm run sync:rosters-2026` (dry-run) or `APPLY_CHANGES=1 npm run sync:rosters-2026` (upserts by gsis, keeps CUT/FA rows, never deletes). `npm run sync:weekly-stats-2026` fills `weekly_stats_2026` once nflverse publishes `stats_player_week_2026.csv` after week 1.
